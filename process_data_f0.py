...
if __name__ == '__main__':
    ...
    roi_mask_bool = {} # dict with key = exp, ch and value = bool array of roi masks, shape: roi_index, x, y ,(z)
    roi_image = {} # dict with key = exp,ch and value = 3D array of mean image data, shape: x,y,(z)
    ...
    for exp_ind, current_series_num in enumerate(series_num): #loop through all series, current_series_num is a list of series to analyze together
        ...
        for series_ind, current_series in enumerate(current_series_num):
            ...
            print('drawing rois on series ' + str(current_series) + 'using channel ' + str(current_channel))
            ...
            if run_gui:
                ...
                os.system('python ' + gui_path
                        ...
                        + ' --rig ' + rig
                        + ' --series_number ' + str(current_series+1) # use first element of tuple to draw rois for current series
                        + ' --image_file_path ' + image_file_path)


            ID = imaging_data.ImagingDataObject(experiment_file_path,
                                                current_series+1, # use first element of tuple to extract rois for current series
                                                quiet=False)

            # Retrieve saved mask region responses from data file

            roi_mask_bool[exp,ch] = ID.getRoiMasks(roi_set_name)['roi_mask'] #shape: roi_index, x, y ,(z)
            roi_image[exp,ch] = ID.getRoiMasks(roi_set_name)['roi_image'] #shape: x,y,(z)

            print('dimensions of roi_image: ' + str(np.shape(roi_image[exp,ch])))
            print('dimensions of roi_mask_bool[exp,ch]: ' + str(np.shape(roi_mask_bool[exp,ch])))

            # figure out if data is volume or slice
            num_spacial_dim = len(roi_image[exp,ch].shape)
            ...
            roi_mask = np.zeros(roi_mask_bool[exp,ch].shape[1:])
            print('dimensions of func data: ' + str(np.shape(roi_mask))) # shape:x,y,(z)

            if num_spacial_dim == 3: #data is a volume
                for roi_ind in range(roi_mask_bool[exp,ch].shape[0]):
                    for i in range(roi_mask_bool[exp,ch].shape[1]):
                        for j in range(roi_mask_bool[exp,ch].shape[2]):
                            for k in range(roi_mask_bool[exp,ch].shape[3]):
                                if roi_mask_bool[exp,ch][roi_ind,i,j,k] == True:
            ...
            if mean_image[exp,ch].shape == roi_mask.shape: #check that mean image and roi mask have same dimensions
            ...
            for slice in range(roi_mask_bool[exp,ch].shape[3]): #loop through z slices
                im = plot_tools.overlayImage(mean_image[exp,ch][:,:,slice], roi_mask_bool[exp,ch][:,:,:,slice], alpha=0.5, colors=None)
                fig = plt.subplot(1,1,1)
                fig.imshow(im)
                if save_figs:
                    fig_name = fig_name_string + '_rois_{}_{}_slice_{}_'.format(exp,ch,slice)
    ...
    for ch in voxel_mean:
        ...
        ### save data out to hdf5
    save_path = os.path.join(experiment_file_directory,'f0_data.pkl')
    f0_data = {}
    f0_data['roi_mask_bool'] = roi_mask_bool
    f0_data['roi_mask'] = roi_mask
    f0_data['roi_image'] = roi_image
    f0_data['image_data'] = image_data
    f0_data['mean_image'] = mean_image
    f0_data['voxel_mean'] = voxel_mean

    with open(save_path, 'wb') as f:
        pickle.dump(f0_data, f)

# To load the dictionary back:
# with open('saved_dictionary.pkl', 'rb') as f:
#     f0_data = pickle.load(f)
# roi_mask_bool = f0_data['roi_mask_bool']
# roi_mask = f0_data['roi_mask']
# roi_image = f0_data['roi_image']
# image_data = f0_data['image_data']
# mean_image = f0_data['mean_image']
# voxel_mean = f0_data['voxel_mean']
