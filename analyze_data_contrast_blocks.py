...
if __name__ == '__main__':
    ...
    for current_series in series_num: #loop through all series
        ...
        for current_channel in func_channels_num: #loop through channels
            ...
            if sn == 'sn1':
                ...
                unique_parameter_values[sn], mean_response[sn,ch], sem_response[sn,ch], trial_response_by_stimulus[sn,ch] = ID.getTrialAverages(roi_data[sn,ch]['epoch_response'], parameter_key='color_max')    
    ...
    repeat_blocks_series = 'sn' + series_num[1] #assume second series is repeat blocks

    random_blocks_series = 'sn' + series_num[2] #assume third series is random blocks
    ...
    # plot 5: individual responses
    sn = random_blocks_series
    fig_name_string = 'individual_responses' 
    fig_format = '.pdf'
    for roi_ind in range(n_roi):
        fh, ax = plt.subplots(len(func_channels_num), len(unique_parameter_values[sn]), figsize=(12, 12*(9/16)),constrained_layout = True)
        for ch_ind, current_channel in enumerate(func_channels_num):
            ...
            for u_ind, up in enumerate(unique_parameter_values[sn]):
                ...
                ax[ch_ind, u_ind].set_xlabel('Time (s)')
                ax[ch_ind, u_ind].set_title('Ch{}, color_max = {}'.format(current_channel,up))
        plt.suptitle('Individual responses {}, roi {}'.format(tag,roi_ind))
    ...
    # plot 7: mean responses - all rois
    sn = random_blocks_series
    fig_name = 'mean_responses_{}_rois'.format(tag)
    fig_format = '.pdf' 
    fh, ax = plt.subplots(len(func_channels_num), len(unique_parameter_values[sn]), figsize=(12, 12*(9/16)),constrained_layout = True)
    for ch_ind, current_channel in enumerate(func_channels_num):
        ch = 'ch' + current_channel
        for u_ind, up in enumerate(unique_parameter_values[sn]):
            ...
            ax[ch_ind, u_ind].set_title('Ch{}, color_max = {}'.format(current_channel,up))
            ax[ch_ind, u_ind].axvspan(run_parameters[sn]['pre_time'], run_parameters[sn]['pre_time'] + run_parameters[sn]['stim_time'], color='gray', alpha=0.2)
    plt.suptitle('Mean responses, all {} rois'.format(tag))
    ...
    # plot 12: combined mean response - all final rois
    if tag == 'final':
        sn = random_blocks_series
        fig_name = 'combined_mean_responses_all_{}_rois'.format(tag) #convert from s to ms
        fig_format = '.pdf' 
        fh, ax = plt.subplots(len(func_channels_num), len(unique_parameter_values[sn]), figsize=(12, 12*(9/16)),constrained_layout = True)
        for ch_ind, current_channel in enumerate(func_channels_num):
            ch = 'ch' + current_channel
