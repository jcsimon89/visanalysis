...
if __name__ == '__main__':
    ...
    # figure out which series is which (search stim + one long flash series)

    search_series = 'sn' + series_num[0]  # assume first series is search stim
    print('search stim series: ' + search_series)

    long_flash_series = 'sn' + series_num[1]  # second series is the long flash
    print('long flash series: ' + long_flash_series)
    ...
    # plot 5: individual responses (long flashes)
    ...
    fig_format = '.pdf'
    for roi_ind in range(n_roi):
        fh, ax = plt.subplots(len(func_channels_num), len(unique_intensity_values[sn]), figsize=(3*12, 12*(9/16)),constrained_layout = True)
        for ch_ind, current_channel in enumerate(func_channels_num):
            ch = 'ch' + current_channel
            for u_ind, up in enumerate(unique_intensity_values[sn]):
    ...
    # plot 6: mean responses (long flashes) - all rois
    sn = long_flash_series
    fig_stim_time = str(int(1000*run_parameters[sn]['stim_time'])) + 'ms'
    ...
    # plot 9: combined mean response (long flashes) - combined mean of all final rois
    if tag == 'final':
        sn = long_flash_series
        fig_stim_time = str(int(1000*run_parameters[sn]['stim_time'])) + 'ms'
