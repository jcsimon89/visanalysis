...
class ImagingDataObject:
    ...
    def __init__(self, file_path, series_number, quiet=False, cfg_dict=None):
        ...
        assert (
            self.file_path.split(".")[-1] == "hdf5"
        ), "file_path must point to an .hdf5 file, \n current file_path is {}".format(
            self.file_path
        )
        assert os.path.exists(
            self.file_path
        ), "No hdf5 file found at \n {}, check your filepath".format(self.file_path)
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            if epoch_run_group is None:
                raise Exception(
                    "No series {} found in {}".format(
                        self.series_number, self.file_path
                    )
                )
    ...
    def getRunParameters(self, param_key=None):
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            if param_key:
                assert (
                    param_key in epoch_run_group.attrs
                ), 'Run parameter "{}" not found in run parameters. \n'.format(
                    param_key
                ) + "Available run parameters are {}".format(
                    [x for x in epoch_run_group.attrs]
                )
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            for epoch in epoch_run_group["epochs"].values():
                if param_key:
                    ...
                    ), 'Epoch parameter "{}" not found in epoch_parameters. \n'.format(
                        param_key
                    ) + "Available epoch_parameters are {}".format(
                        [x for x in epoch.attrs]
                    )
    ...
    def getEpochParameterDicts(self, target_keys=None):
        ...
        for ep in self.getEpochParameters():
            if (
                target_keys is None
            ):  # Default: any params with "current" or "component" in them, as in "current_intensity"
                new_keys = [
                    key for key in ep.keys() if "current" in key or "component" in key
                ]
            else:
    ...
    def getExperimentMetadata(self, metadata_key=None):
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            if metadata_key:
                assert (
                    metadata_key in experiment_file.attrs
                ), 'metadata_key "{}" not found in metadata. \n'.format(
                    metadata_key
                ) + "Available metadata keys are {}".format(
                    [x for x in experiment_file.attrs]
                )
    ...
    def getAcquisitionMetadata(self, metadata_key=None):
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            if metadata_key:
                assert (
                    metadata_key in acquisition_group.attrs
                ), 'metadata_key "{}" not found in metadata. \n'.format(
                    metadata_key
                ) + "Available metadata keys are {}".format(
                    [x for x in acquisition_group.attrs]
                )
    ...
    def getResponseTiming(self):
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            response_timing = {}
            response_timing["time_vector"] = acquisition_group.get("time_points")[
                :
            ]  # sec
            response_timing["sample_period"] = acquisition_group.attrs[
                "sample_period"
            ]  # sec
    ...
    def getStimulusTiming(self, plot_trace_flag=False):
        ...
        for ch in range(num_channels):
            ...
            b, a = signal.butter(
                4, 10 * self.command_frame_rate, btype="low", fs=sample_rate
            )
            ...
            ups = (
                np.where(
                    np.logical_and(V_orig < self.threshold, V_shift >= self.threshold)
                )[0]
                + 1
            )
            downs = (
            ...
            stimulus_end_frames = np.append(
                np.where(np.diff(frame_times) > minimum_epoch_separation)[0],
                len(frame_times) - 1,
            )
            stimulus_start_times = (
                frame_times[stimulus_start_frames] / sample_rate
            )  # datapoints -> sec
            stimulus_end_times = (
                frame_times[stimulus_end_frames] / sample_rate
            )  # datapoints -> sec
            ...
            for s_ind, ss in enumerate(stimulus_start_frames):
                ...
                frame_durations.append(
                    frame_len[good_frame_inds]
                )  # only include non-dropped frames in frame rate calc
            ...
            if plot_trace_flag:
                ...
                ax.plot(
                    stimulus_start_times,
                    self.threshold * np.ones(stimulus_start_times.shape),
                    "go",
                )
                ax.plot(
                    stimulus_end_times,
                    self.threshold * np.ones(stimulus_end_times.shape),
                    "ro",
                )
                ax.plot(
                    dropped_frame_times / sample_rate,
                    1 * np.ones(dropped_frame_times.shape),
                    "rx",
                )
                ax.set_title(
                    "Ch. {}: Frame rate = {:.2f} Hz".format(ch, frame_rate), fontsize=12
                )
                ...
                ymin = np.min(
                    [0.9 * run_parameters["stim_time"], np.min(stim_durations)]
                )
                ymax = np.max(
                    [1.1 * run_parameters["stim_time"], np.max(stim_durations)]
                )
            ...
            if self.quiet:
                ...
                print(
                    "===================TIMING: Channel {}======================".format(
                        ch
                    )
                )
                print(
                    "{} Stims presented (of {} parameterized)".format(
                        len(stim_durations), len(epoch_parameters)
                    )
                )
                ...
                print(
                    "Dropped {} / {} frames ({:.2f}%)".format(
                        dropped_frames,
                        total_frames,
                        100 * dropped_frames / total_frames,
                    )
                )
                print("==========================================================")
    ...
    def getBehaviorData(self, stimulus_timing, use_camera_timestamps=False, quiet=True):
        ...
        with h5py.File(self.file_path, "r") as experiment_file:
            ...
            if "fictrac_data" in behavior_group.keys():
                ...
                fictrac_data = fictrac_data.astype(
                    {"frame_count": int, "sequence_number": int}
                )
            ...
            if "log_lines" in behavior_group.keys():
                ...
                set_pos_0_groups = [
                    (
                        log_group[line_name]["set_pos_0"],
                        log_group[line_name].attrs["ts"],
                    )
                    for line_name in log_group.keys()
        ...
        if len(stim_start_times) < n_epochs:
            print(
                "CAUTION! stimulus_timing has length less than number of epochs. Trimming # of epochs."
            )
        ...
        for e in range(n_epochs):
            ...
            while (
                ...
                and fictrac_timestamps[ts_pointer] <= current_stim_end_time
            ):
    ...
    def getRoiResponses(
        ...
        if background_subtraction:
            with h5py.File(self.file_path, "r") as experiment_file:
                find_partial = functools.partial(
                    h5io.find_series, sn=self.series_number
                )
                roi_parent_group = experiment_file.visititems(find_partial)[roi_prefix]
                bg_roi_group = roi_parent_group["bg"]
                bg_roi_response = list(bg_roi_group.get("roi_response")[:])

            roi_data["roi_response"] = [
                x - np.squeeze(bg_roi_response) for x in roi_data["roi_response"]
            ]
    ...
    def getEpochGroupingsByParameters(self, parameter_key=None, replace_parameter_value=None):
        ...
        unique_parameter_values = [
            list(s) for s in set(tuple(pv) for pv in parameter_values)
        ]
        ...
        epoch_inds = [
            np.where([pv == up for pv in parameter_values])[0]
    ...
    def getTrialAverages(self, epoch_response_matrix, parameter_key=None, replace_parameter_value=None, min_samples=0):
        ...
        )  # n_regions x stim condition x time
    ...
    def getResponseAmplitude(self, epoch_response_matrix, metric="max"):
        ...
        pre_frames = int(
            run_parameters["pre_time"] / response_timing.get("sample_period")
        )

        if metric == "max":
            response_amplitude = np.nanmax(
                epoch_response_matrix[..., pre_frames:], axis=-1
            )
        elif metric == "mean":
            response_amplitude = np.nanmean(
                epoch_response_matrix[..., pre_frames:], axis=-1
            )
        elif metric == "min":
            response_amplitude = np.nanmin(
                epoch_response_matrix[..., pre_frames:], axis=-1
            )
    ...
    def generateRoiMap(self, roi_name, scale_bar_length=0, z=None):
        ...
        if scale_bar_length > 0:
            microns_per_pixel = float(
                self.getAcquisitionMetadata()["micronsPerPixel_XAxis"]
            )
            plot_tools.addImageScaleBar(
                ax, new_image, scale_bar_length, microns_per_pixel, "lr"
            )
